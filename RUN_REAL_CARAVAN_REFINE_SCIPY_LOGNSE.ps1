...
Write-Host "1) Ensure minimal CARAVAN cache for one basin (PET=FAO)"
...
Write-Host "2) Calibration refine (scipy, LOGNSE loss)"
conda run -n $CondaEnv python scripts/run_xaj_calibration.py --config configs/real_caravan_camels_01013500_refine_scipy_lognse.yaml

Write-Host "3) Evaluation (test period)"
$caliDir = "results/real_caravan_camels_01013500_refine_scipy_lognse/xaj_mz_scipy"
