...
# Check availability without importing packages (avoid side effects at import time).
HYDRODATASET_AVAILABLE = importlib.util.find_spec("hydrodataset") is not None
HYDRODATASOURCE_AVAILABLE = importlib.util.find_spec("hydrodatasource") is not None
...
class UnifiedDataLoader:
    ...
    # hydrodataset.Caravan maps standard names, not raw NetCDF names.
    # Temperature aliases are stripped before read_ts_xrdataset and re-attached
    # from cache (see _attach_temperature).
    _TEMPERATURE_REQUEST_ALIASES = {
        "temperature",
        "temperature_mean",
        "temperature_2m_mean",
        "tmean",
    ...
    def _get_default_data_path(self) -> str:
        ...
        try:
            setting_file = self._find_setting_file()
            if setting_file and os.path.exists(setting_file):
                with open(setting_file, "r", encoding="utf-8") as f:
                    settings = yaml.safe_load(f)

                if settings and "local_data_path" in settings:
                    # Resolve relative paths relative to setting file directory
                    base_dir = Path(setting_file).resolve().parent
                    local_data_path = settings.get("local_data_path", {})
    ...
    def _create_datasource(self) -> Any:
        ...
        if category == "hydrodataset":
            ...
            # Some datasets (e.g., Caravan) accept extra init kwargs such as `region`.
            init_kwargs = (self.config.get("datasource_kwargs") or {}).copy()
            try:
                ds = dataset_class(self.data_path, **init_kwargs)
            except TypeError:
                # Backward-compatible fallback for dataset classes that only accept `data_path`.
                ds = dataset_class(self.data_path)
            self._ensure_caravan_temperature_mapping(ds)
            return ds
    ...
    def load_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Tuple[np.ndarray, np.ndarray]
            ...
            - p_and_e: [time, basin, features=2, 3, or 4]
            - qobs: Observed streamflow [time, basin, features=1]

        Notes
        -----
        - Continuous without T: features=2 (precipitation, PET) — xaj_mz unchanged
        - Continuous with T: features=3 (precipitation, PET, temperature °C)
        - Flood event data: features=3 (precipitation, PET, flood_event_marker)
        """
    ...
    def _load_continuous_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        hydro_vars = self._hydrodataset_var_list()
        try:
            ...
            # Cache may lack temperature; fall back to P/PET/Q only.
            msg = str(exc).lower()
            if "temperature" in msg or "missing from the cache" in msg:
                hydro_vars = [
                    v
                    for v in hydro_vars
                    if str(v) not in self._TEMPERATURE_REQUEST_ALIASES
                    and str(v) != "temperature_mean"
                ]
                ts_data = self.datasource.read_ts_xrdataset(
                    gage_id_lst=self.basin_ids,
                    t_range=self.time_range,
        ...
        xr_dataset = self._deduplicate_basin_index(xr_dataset)
        xr_dataset = self._attach_temperature(xr_dataset)
        # Materialize so NetCDF/dask file handles can close (Windows hang/crash).
    ...
    def _xrdataset_to_ndarray(
        ...
        temp_var = self._find_variable_name(
            xr_dataset, self.VAR_MAPPING["temperature"]
        )

        if flood_event_var:
            # Event data with flood_event: [time, basin, features=3]
            # Keep marker as feature 2 (do not insert temperature here).
            flood_event = (
                xr_dataset[flood_event_var].transpose("time", "basin").values
            )
            p_and_e = np.stack([prcp, pet, flood_event], axis=2)
        elif temp_var:
            # Continuous with air temperature: [time, basin, features=3]
            temp = xr_dataset[temp_var].transpose("time", "basin").values
            p_and_e = np.stack([prcp, pet, temp], axis=2)
        else:
    ...
    def _hydrodataset_var_list(self) -> List[str]:
        """Map config variable names to hydrodataset standard names.

        Temperature aliases become ``temperature_mean``. If the config omitted
        T, still request it when the Caravan mapping exists so p_and_e can be
        3 features without a second cache open (HDF5 crash on Windows).
        """
        hydro_vars: List[str] = []
        for v in self.variables:
            key = str(v)
            if key in self._TEMPERATURE_REQUEST_ALIASES:
                if "temperature_mean" not in hydro_vars:
                    hydro_vars.append("temperature_mean")
            else:
                hydro_vars.append(key)
        mapping = getattr(self.datasource, "_dynamic_variable_mapping", None)
        if (
            isinstance(mapping, dict)
            and "temperature_mean" in mapping
            and "temperature_mean" not in hydro_vars
        ):
            hydro_vars.append("temperature_mean")
        if not hydro_vars:
            hydro_vars = [
    ...
    @staticmethod
    def _ensure_caravan_temperature_mapping(datasource) -> None:
        """Caravan maps Tmax/Tmin but not catchment-mean T; add Tmean → cache name."""
        mapping = getattr(datasource, "_dynamic_variable_mapping", None)
        if not isinstance(mapping, dict):
            return
        if "temperature_mean" in mapping:
            return
        mapping = dict(mapping)
        mapping["temperature_mean"] = {
            "default_source": "each_region",
            "sources": {
                "each_region": {
                    "specific_name": "temperature_2m_mean",
                    "unit": "°C",
                }
            },
        }
        datasource._dynamic_variable_mapping = mapping

    def _attach_temperature(self, xr_dataset: xr.Dataset) -> xr.Dataset:
        """No-op if T already loaded; do not reopen hydrodataset cache files."""
        return xr_dataset

    @staticmethod
    def _deduplicate_basin_index(xr_dataset: xr.Dataset) -> xr.Dataset:
        """Drop duplicate basin entries from merged CARAVAN cache batches."""
        if "basin" not in xr_dataset.dims:
            return xr_dataset
        basin_vals = [str(b) for b in xr_dataset["basin"].values.tolist()]
        if len(basin_vals) == len(set(basin_vals)):
            return xr_dataset
        _, keep_idx = np.unique(basin_vals, return_index=True)
        keep_idx = np.sort(keep_idx)
        deduped = xr_dataset.isel(basin=keep_idx)
        dropped = sorted(set(basin_vals) - set(str(b) for b in deduped["basin"].values.tolist()))
        if dropped:
            print(
                "Warning: duplicate basin rows in cache merge; kept first occurrence per basin."
            )
        return deduped
