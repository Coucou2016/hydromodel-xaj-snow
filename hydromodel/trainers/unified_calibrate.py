...
def calibrate(config, **kwargs) -> Dict[str, Any]:
    ...
    **kwargs
    ...
    # Extract model config (flattened yaml puts source_type etc. at model_cfgs top level)
    model_cfgs = config["model_cfgs"]
    extra_model = {
        k: v
        for k, v in model_cfgs.items()
        if k not in ("model_name", "model_params", "output_variable")
    }
    model_config = {
        "name": model_cfgs.get("model_name"),
        **extra_model,
        **(model_cfgs.get("model_params") or {}),
    }
...
def _calibrate_with_scipy(model_setup, algorithm_config, output_dir, basin_id):
    ...
    # yaml may parse scientific notation as string in some cases; normalize types
    try:
    ...
    def _load_initial_from_file(path: str) -> np.ndarray | None:
        """Load normalized initial parameters from a previous calibration JSON.

        Expected format: { "<basin_id>": {"best_params": {"<model_name>": {param: value}}}}
        ...
        if basin_key not in data:
            return None
        best_params = (
            data[basin_key]
            .get("best_params", {})
            .get(model_setup.model_name, None)
        )
        if not isinstance(best_params, dict):
            return None
        x0 = np.array(
            [float(best_params.get(name, np.mean(bounds[i]))) for i, name in enumerate(param_names)],
            dtype=float,
        )
        # Clip to bounds
        for i, (low, high) in enumerate(bounds):
            x0[i] = float(np.clip(x0[i], low, high))
        return x0

    # Initialize from middle of bounds (default) or from previous run
    initial_params = np.array([np.mean(bound) for bound in bounds], dtype=float)
    initial_from = algorithm_config.get("initial_params_from")
    if isinstance(initial_from, str) and initial_from.strip():
        cand = initial_from.strip()
        # allow passing a directory
        if os.path.isdir(cand):
            cand = os.path.join(cand, "calibration_results.json")
        loaded = _load_initial_from_file(cand)
        if loaded is not None:
            initial_params = loaded

    # Multi-start (optional): run several local optimizations from different initial points.
    # This often improves robustness vs a single start, especially for non-convex objectives.
    n_starts = algorithm_config.get("n_starts", 1)
    init_strategy = str(algorithm_config.get("init_strategy", "midpoint")).strip().lower()
    seed = algorithm_config.get("seed", 0)
    try:
    ...
    if n_starts > 1:
        print(f"   • Multi-starts: {n_starts} (strategy={init_strategy}, seed={seed})\n")
    else:
        print("")
    ...
    # Run optimization (possibly multi-start)
    ...
    x0_list: list[np.ndarray] = [initial_params]
    if n_starts > 1:
        if init_strategy in {"random", "rand"}:
            x0_list.extend(_random_x0() for _ in range(n_starts - 1))
        else:
            # default: keep using the same x0 (midpoint / loaded) but jitter slightly
            for _ in range(n_starts - 1):
                x = initial_params + rng.normal(loc=0.0, scale=0.05, size=initial_params.shape)
                for i, (low, high) in enumerate(bounds):
                    x[i] = float(np.clip(x[i], low, high))
                x0_list.append(x.astype(float))

    best_result = None
    best_fun = float("inf")
    all_histories: list[pd.DataFrame] = []

    for start_idx, x0 in enumerate(x0_list, start=1):
        ...
        # Create progress bar
        pbar = tqdm(total=max_iterations, desc=f"Scipy {start_idx}/{len(x0_list)}", unit="iter")
        ...
        if iteration_history:
            all_histories.append(pd.DataFrame(iteration_history))

        # Track best across starts (even if result.success is False, we can still compare fun)
        try:
    ...
    if all_histories:
        df_history = pd.concat(all_histories, axis=0, ignore_index=True)
        df_history.to_csv(results_file, index=False)
        print(f"Results saved to: {results_file}")

    # Process results
    print(f"\n✅ {'='*60}")
    result = best_result if best_result is not None else None
    if result is not None and getattr(result, "x", None) is not None:
        ...
        if bool(getattr(result, "success", False)):
            ...
            print(f"⚠️  Scipy optimization ended without success flag for basin: {basin_id}")
            print(f"   📊 Message: {getattr(result, 'message', '')}")
        print(f"   📊 Total iterations: {getattr(result, 'nit', 'NA')}")
        print(f"   📊 Function evaluations: {getattr(result, 'nfev', 'NA')}")
        try:
            print(f"   📊 Best objective value: {float(result.fun):.6f}")
        except Exception:
            print(f"   📊 Best objective value: NA")
