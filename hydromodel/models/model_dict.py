...
def _to_1d(obs, sim):
    """Flatten common hydromodel obs/sim arrays to 1D."""
    o = np.asarray(obs)
    s = np.asarray(sim)
    return o.reshape(-1), s.reshape(-1)


def nse_loss(obs, sim):
    """Minimization-friendly NSE loss: 1 - NSE (lower is better)."""
    o, s = _to_1d(obs, sim)
    return float(1.0 - spotpy_obj.nashsutcliffe(o, s))


def kge_loss(obs, sim):
    """Minimization-friendly KGE loss: 1 - KGE (lower is better)."""
    o, s = _to_1d(obs, sim)
    return float(1.0 - spotpy_obj.kge(o, s))


def lognse_loss(obs, sim):
    """Minimization-friendly log NSE loss: 1 - logNSE (lower is better)."""
    o, s = _to_1d(obs, sim)
    return float(1.0 - spotpy_obj.lognashsutcliffe(o, s))


def nse_kge_loss(obs, sim):
    """Composite loss: (1-NSE) + (1-KGE) (lower is better)."""
    o, s = _to_1d(obs, sim)
    nse = spotpy_obj.nashsutcliffe(o, s)
    kge = spotpy_obj.kge(o, s)
    return float((1.0 - nse) + (1.0 - kge))
