> **Public research snapshot (XAJ-Snow pilot):** https://github.com/Coucou2016/hydromodel-xaj-snow  
> This repository publishes source code, curated figures/publications, and docs for an exploratory XAJ–snow pilot on Caravan/CAMELS. **Large hydrodata / `_portable_data` / NetCDF caches are not included** — prepare data locally. Publication drafts are **pilot manuscripts, not final conclusions**. See [`docs/local/github_public_repo.md`](docs/local/github_public_repo.md) for include/exclude details and suggested reading paths.
