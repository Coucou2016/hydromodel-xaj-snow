...
Write-Host "1) Build minimal CARAVAN cache for one basin"
...
Write-Host "2) Calibration (medium budget, KGE loss)"
conda run -n $CondaEnv python scripts/run_xaj_calibration.py --config configs/real_caravan_camels_01013500_kge_medium.yaml

Write-Host "3) Evaluation (test period)"
$caliDir = "results/real_caravan_camels_01013500_kge_medium/xaj_mz_SCE_UA"
