...
Write-Host "1) Rebuild minimal CARAVAN cache for one basin (PET = ERA5_LAND)"
$tsCache = "_portable_data\.cache\caravan_camels_timeseries_batch_camels_01013500_camels_01013500.nc"
if (Test-Path $tsCache) {
  Remove-Item $tsCache -Force
}
conda run -n $CondaEnv python scripts/build_caravan_minicache.py --data-root $dataRoot --region-folder $regionFolder --basin-id $basinId --t-range 1985-10-01 2014-09-30 --pet-preference ERA5_LAND

Write-Host "2) Calibration refine (scipy, NSE loss)"
conda run -n $CondaEnv python scripts/run_xaj_calibration.py --config configs/real_caravan_camels_01013500_refine_scipy_nse_era5pet.yaml

Write-Host "3) Evaluation (test period)"
$caliDir = "results/real_caravan_camels_01013500_refine_scipy_era5pet/xaj_mz_scipy"
