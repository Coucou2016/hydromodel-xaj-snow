...
Write-Host "1) Build minimal CARAVAN cache for one basin (avoid full regional caching)"
...
Write-Host "2) Calibration (stronger budget, KGE loss)"
conda run -n $CondaEnv python scripts/run_xaj_calibration.py --config configs/real_caravan_camels_01013500_stronger.yaml

Write-Host "3) Evaluation (test period)"
$caliDir = "results/real_caravan_camels_01013500_stronger/xaj_mz_SCE_UA"
...
Write-Host "5) Basin overview map (boundary + true gauge point)"
